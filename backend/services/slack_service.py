...
class SlackService:
    def __init__(self, webhook_url: str):
        ...
        self.enabled = bool(webhook_url and "placeholder" not in webhook_url and webhook_url.strip() != "")
    
    async def send(self, message: dict) -> bool:
        """Send a Slack message. Returns True on success, False if disabled or failed."""
        if not self.enabled:
            logger.info(f"[MOCK] Slack notification: {message.get('text') or message.get('attachments')}")
            return True
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(self.webhook_url, json=message, timeout=5.0)
                if response.status_code != 200:
                    logger.warning(f"Slack notification returned non-200 status: {response.status_code}")
                return response.status_code == 200
        except Exception as e:
            logger.warning(f"Slack notification failed: {e}")
            return False

    async def notify_pipeline_result(self, branch: str, status: str, duration: int, run_url: str, commit: str = "N/A") -> bool:
                            ...
                            "elements": [
                                {
                                    ...
                                    "url": run_url,
                                    "style": "primary" if status.lower() == "success" else "danger"
                                }
                            ]
                        }
                    ]
    ...
    async def notify_system_alert(self, alert_type: str, message_text: str, severity: str) -> bool:
