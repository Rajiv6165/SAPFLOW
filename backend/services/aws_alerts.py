...
class AWSAlertsService:
    def __init__(self):
        ...
        if settings.AWS_ACCESS_KEY_ID and settings.AWS_SECRET_ACCESS_KEY:
            ...
            self.sns = None
            logger.warning(
                "AWS credentials not configured. AWS alerts service will be disabled."
            )
...
Timestamp: {self._get_timestamp()}
    ...
    async def record_pipeline_result(self, status: str, branch: str, duration: int):
        ...
        try:
            ...
            logger.info(
                f"Custom CloudWatch metric recorded for pipeline run completion: status={status}"
            )
