...
def _init_mock_runs():
    ...
    for idx, item in enumerate(setup):
        ...
        updated_at = (
            triggered_at + timedelta(seconds=item["duration"])
            if item["duration"]
            else now
        )
...
class GitHubService:
    ...
    async def get_run_jobs(self, run_id: Any) -> List[Dict[str, Any]]:
        ...
        if self._is_mock_mode():
            logger.info(
                f"GitHub API in mock mode. Returning mock jobs for run {normalized_id}."
            )
                            ...
                            "steps": [
                                {
                                    "name": step["name"],
                                    "status": self._map_status(
                                        step["status"], step.get("conclusion")
                                    ),
                                }
    ...
    async def trigger_workflow(
        self, workflow_id: str, branch: str, inputs: Optional[Dict[str, Any]] = None
    ) -> bool:
        if self._is_mock_mode():
            logger.info(
                f"GitHub API in mock mode. Simulating dispatch trigger for {workflow_id} on {branch}."
            )
            ...
            for run in runs:
                ...
                if triggered_at and completed_at:
                    duration_seconds = int(
                        (completed_at - triggered_at).total_seconds()
                    )
    ...
    def _generate_mock_jobs(self, run_id: int) -> List[Dict[str, Any]]:
        ...
        job_steps = {
            ...
            "report-to-sapflow": ["Report pipeline result to SAPFlow"],
        }
        ...
        for job_name, (status, conclusion, step_statuses) in jobs_status.items():
            ...
            duration = (
                ...
                if status == "success"
                else (random.randint(5, 15) if status == "failed" else 0)
            )
