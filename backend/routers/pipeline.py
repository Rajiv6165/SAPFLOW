...
async def get_pipeline_status(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        result = await db.execute(
            select(PipelineRun).order_by(PipelineRun.triggered_at.desc()).limit(10)
        )
        ...
        for i, run in enumerate(runs):
            ...
            if i < 3 and github_service:
                try:
                    ...
                    jobs = [
                        {"name": j["name"], "status": j["status"]} for j in raw_jobs
                    ]
...
async def trigger_pipeline(request: Request, payload: PipelineTriggerRequest):
    ...
    if not success:
        raise HTTPException(
            status_code=500, detail=f"Failed to trigger pipeline for branch {branch}"
        )
...
async def sync_pipeline(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return {
            "synced": synced_count,
            "message": f"Synced {synced_count} runs from GitHub",
        }
    except Exception as e:
